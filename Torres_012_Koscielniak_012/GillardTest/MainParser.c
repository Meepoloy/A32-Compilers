...
/* External objects */
extern gillard_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern gillard_intg line; /* source code line number - defined in scanner.c */
...
extern gillard_void startParser(gillard_void);
extern gillard_intg startScanner(ReaderPointer sc_buf);

static gillard_void printParserError(gillard_char* fmt, ...);
static gillard_void displayParser(ReaderPointer ptrBuffer);
static gillard_long getParserFileSize(gillard_char* fname);
static gillard_void callGarbageCollector(gillard_void);
...
gillard_void printParserError(gillard_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(gillard_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
gillard_long getParserFileSize(gillard_char* fname) {
	FILE* input;
	gillard_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
gillard_void displayParser(ReaderPointer ptrBuffer) {
...
gillard_void callGarbageCollector(gillard_void) {
